    ...
    655: function() {
        ...
        !function() {
            var t = window.document.currentScript
              ...
              , o = "".concat(new URL(t.src).origin, "/api/v1/update-event")
              , i = t.getAttribute("info-hash") || !1
              , a = function(e) {
              ...
              , r = function(e, r) {
                if (/^localhost$|^127(\.[0-9]+){0,2}\.[0-9]+$|^\[::1?\]$/.test(window.location.hostname) || "file:" === window.location.protocol)
                    return console.log("Event ignored due to environment being localhost"),
                    0;
                if (window._phantom || window.__nightmare || window.navigator.webdriver || window.Cypress)
                    return console.log("Event ignored due to testing or scraper env"),
                    0;
                if ("true" === window.localStorage.ignore_crt_track)
                    return console.log("Event ignored due to rejected track"),
                    0;
                var d = t && t.getAttribute("info-append")
                  , c = t && t.getAttribute("info-except");
                if ("pageview" === e) {
                    var w = !d || d && d.split(",").some(a)
                      , s = c && c.split(",").some(a);
                    if (!w || s)
                        return console.log("Event ignored due to exclusion rule applied"),
                        0
                }
                var l = {
                    name: e,
                ...
                r && r.meta && (l.meta = JSON.stringify(r.meta)),
                r && r.props && (l.props = r.props),
                i && (l.hash = 1),
                u.open("POST", n, !0),
            ...
            for (var c = 0; c < d.length; c++)
                r.apply(e, d[c]);
            var w, s = function() {
                (i || w !== window.location.pathname) && (w = window.location.pathname,
                r("pageview"))
            };
            if (i)
