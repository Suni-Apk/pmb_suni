                        ...
                        <form action="{{ route('admin.course.update', $course->id) }}" method="POST">
                            ...
                            {{-- <div class="form-group mb-3">
                                ...
                                <div class="input-group">
                                    ...
                                    <input type="number" step="500" name="amount" id="amount" min="0" class="form-control" value="{{ $course->administrasi->amount }}" required>
                                </div>
                                @error('amount')
                                    <div class="text-danger text-sm">{{ $message }}</div>
                                @enderror
                            ...
                            @if ($course->notes)
                                <div class="form-group mb-3">
                                    ...
                                    <div id="notesContainer">
                                        @foreach ($course->notes as $value)
                                            @if ($loop->first)
                                            <div class="note-input-container input-group mb-2">
                                                <input type="text" name="notes[]" class="form-control" value="{{ $value }}">
                                            </div>
                                            @else
                                            <div class="note-input-container input-group mb-2 flex-row-reverse">
                                                <input type="text" name="notes[]" class="form-control" value="{{ $value }}">
                                                <button type="button" onclick="removeNotes(this)" class="btn btn-light shadow-none input-group-text px-3">
                            ...
                            <div class="form-group mb-3">
                                ...
                                <textarea name="desc" id="desc" class="form-control min-height-200">{{ $course->descProgram->desc }}</textarea>
                            </div>
